...
display_element_reasons <- function(country, data, args){
  ...
  q <- ggplot(temp, aes(x = where_from, y = value),guide=FALSE) + facet_wrap(~names) + geom_bar(aes(fill = variable), position = "dodge", stat="identity") + guides(fill=FALSE, variable=FALSE) + theme(legend.position = "bottom")
  print(q)
}
